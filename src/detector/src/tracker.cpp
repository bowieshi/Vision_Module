...
void Tracker::update(const detector::Armors & armors_msg)
{
    // KF predict
    Eigen::VectorXd ekf_prediction = ekf.predict();
    ROS_INFO("EKF predict");
...
void Tracker::initEKF(const detector::Armor & a)
{
...
void Tracker::updateArmorsNum(const detector::Armor & armor)
{
    if (armor.type == (int)ArmorType::LARGE && (tracked_id == 3 || tracked_id == 4 || tracked_id == 5)) {
        tracked_armors_num = ArmorId::BALANCE_2;
    }
    else if (tracked_id == 8) { // outpost
        tracked_armors_num = ArmorId::OUTPOST_3;
    }
    else {
        tracked_armors_num = ArmorId::NORMAL_4;
    }
}

void Tracker::handleArmorJump(const detector::Armor & current_armor)
{
    double yaw = orientationToYaw(current_armor.pose.orientation);
    target_state(6) = yaw;
    updateArmorsNum(current_armor);
    // Only 4 armors has 2 radius and height
    if (tracked_armors_num == ArmorId::NORMAL_4) {
